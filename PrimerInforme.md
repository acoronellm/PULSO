...
Adicionalmente, el desarrollo seguirá un workflow de ingeniería de software orientado a mantener la trazabilidad y calidad de los cambios realizados. Las tareas serán gestionadas mediante un backlog y desarrolladas en ramas independientes de GitHub, posteriormente integradas mediante pull requests y revisión por parte de otro integrante del equipo. Este proceso estará acompañado por prácticas de **CI/CD (Integración Continua y Entrega/Despliegue Continuos)** mediante GitHub Actions, que permitirán automatizar la ejecución de pruebas, validaciones de calidad, integración de cambios y, cuando corresponda, el despliegue de las versiones aprobadas.

Para los componentes relacionados con Machine Learning se incorporarán prácticas de MLOps, principalmente en lo relacionado con la trazabilidad de los datos, el entrenamiento, la evaluación y el versionamiento de los modelos. Esto permitirá identificar qué versión del modelo se utilizó, con qué datos fue entrenado, qué resultados obtuvo y bajo qué condiciones fue generado. De esta manera, el enfoque metodológico combinará el prototipado iterativo, CRISP-DM, prácticas de integración continua y principios básicos de MLOps.
...
Además de evaluar el desempeño predictivo, se registrarán los resultados obtenidos para facilitar la comparación entre diferentes versiones de los modelos y mantener la trazabilidad del proceso experimental.
...
Finalmente, los componentes desarrollados serán integrados en la plataforma web PULSO. Se incorporarán el modelo predictivo, el mecanismo de explicabilidad, la simulación y el sistema RAG, conformando una versión funcional del MVP. En esta etapa se realizarán pruebas de integración y funcionamiento para identificar posibles errores y realizar los ajustes finales. Asimismo, se verificará el correcto funcionamiento del workflow de integración continua y se establecerá el versionamiento de la versión del modelo utilizada por la plataforma.
...
La validación de PULSO se realizará de manera progresiva durante las diferentes iteraciones del proyecto, con el propósito de identificar errores, verificar el cumplimiento de los requerimientos y realizar ajustes antes de avanzar hacia las siguientes etapas. La estrategia de validación contemplará tanto el desempeño de los modelos de Machine Learning como el funcionamiento de la plataforma, la integración entre sus componentes, la calidad de las explicaciones, el comportamiento de las simulaciones y la pertinencia de la información proporcionada mediante el componente RAG.

En las primeras iteraciones se validará la calidad y pertinencia de los conjuntos de datos seleccionados para el desarrollo del modelo. Para ello, se analizarán aspectos como la cantidad de registros, tipos de variables, valores faltantes, distribución de las clases, posibles valores atípicos, correlaciones y representatividad de los datos respecto a la población para la cual se pretende realizar la estimación. Asimismo, se verificará que las transformaciones y procesos de preparación de datos puedan reproducirse bajo las mismas condiciones.

Durante las etapas de modelado y evaluación se comprobará el desempeño de los diferentes modelos de Machine Learning mediante conjuntos de datos separados para entrenamiento y prueba. Se utilizarán métricas como precisión, sensibilidad, especificidad, F1-score, ROC-AUC y matriz de confusión, complementadas con métricas de calibración cuando sean pertinentes. La selección del modelo no dependerá únicamente de una métrica individual, sino de una evaluación conjunta de su capacidad de discriminación, calibración, estabilidad e interpretabilidad. Los modelos seleccionados serán registrados y versionados junto con la información necesaria para identificar los datos, configuraciones, métricas y condiciones utilizadas durante su entrenamiento.

La funcionalidad de explicabilidad será validada mediante SHAP, verificando que las variables mostradas como principales contribuyentes correspondan con las variables utilizadas por el modelo y que las explicaciones sean presentadas de manera comprensible para usuarios no especializados. También se revisará que las explicaciones representen la contribución de las variables a la predicción sin presentarlas como relaciones causales. De forma complementaria, se evaluará la explicabilidad global mediante el análisis del comportamiento general del modelo y el ranking de las variables más relevantes.

La funcionalidad de simulación será validada comprobando que el sistema permita modificar únicamente las variables definidas para este propósito, genere nuevamente la estimación a partir de los valores modificados y conserve el resultado inicial para realizar una comparación directa. Se verificará que los escenarios actuales e hipotéticos sean diferenciados claramente en la interfaz y que la plataforma comunique que los cambios observados corresponden a variaciones en la predicción del modelo y no constituyen garantías sobre cambios en el riesgo clínico real.

El componente RAG será validado mediante pruebas sobre las consultas y respuestas generadas, verificando que la información presentada provenga de las guías clínicas y fuentes médicas seleccionadas para el corpus documental. Se evaluará la pertinencia de los documentos recuperados, la coherencia de las respuestas y el uso de un lenguaje sencillo y comprensible. También se comprobará que las respuestas mantengan su carácter informativo y preventivo y que no generen diagnósticos, prescripciones o indicaciones que excedan el alcance definido para PULSO.

#### 7.3.1 Integración continua y validación del workflow

La validación técnica de PULSO estará integrada al workflow de desarrollo mediante prácticas de integración continua. Cada funcionalidad será desarrollada en una rama independiente de GitHub y, una vez terminada, será incorporada mediante una solicitud de incorporación (pull request). Antes de integrar los cambios a la rama principal, otro integrante del equipo revisará el código y verificará el cumplimiento de los criterios de aceptación establecidos para la tarea.

La creación o actualización de cada pull request activará automáticamente un flujo mediante GitHub Actions. Este flujo realizará la instalación de dependencias, comprobaciones de calidad del código y ejecución de pruebas automatizadas. Estas pruebas incluirán pruebas unitarias, funcionales, de integración y de regresión, además de las comprobaciones necesarias para verificar que el proyecto pueda ser compilado y ejecutado correctamente. Los cambios no serán incorporados a la rama principal cuando las pruebas automáticas o la revisión correspondiente presenten resultados no satisfactorios.
...
Para los componentes relacionados con Machine Learning, el workflow incluirá adicionalmente prácticas básicas de MLOps orientadas a garantizar la trazabilidad y reproducibilidad del proceso. Se registrarán las versiones de los datos utilizados, las configuraciones de entrenamiento, las métricas obtenidas y las versiones de los modelos generados. Esto permitirá identificar qué modelo fue utilizado para una determinada predicción y facilitará la reproducción del proceso cuando sea necesario entrenar o incorporar una nueva versión.

Después de que los cambios sean integrados, se realizará una validación del MVP en un entorno de prueba para comprobar el funcionamiento conjunto de la plataforma. Los errores, fallos y observaciones encontrados durante esta etapa serán registrados como nuevas tareas y posteriormente incorporados al backlog para su priorización. De esta manera, la validación se convierte en un proceso continuo que alimenta las siguientes iteraciones del proyecto.

Finalmente, se realizarán pruebas de usabilidad con usuarios potenciales para evaluar aspectos como la facilidad de navegación, comprensión del lenguaje utilizado, interpretación de la estimación de riesgo, claridad de las explicaciones y comprensión de las comparaciones entre escenarios. Los resultados de estas pruebas, junto con las validaciones técnicas, funcionales y de los modelos, serán utilizados para realizar los ajustes finales del MVP.

En conjunto, la estrategia establece un ciclo continuo de **planificación, desarrollo, pruebas, revisión, integración, validación y mejora**. La incorporación de integración continua, versionamiento de modelos y prácticas básicas de MLOps permitirá que el desarrollo de PULSO sea trazable, reproducible y controlado, mientras que las pruebas técnicas, funcionales y de usabilidad permitirán verificar progresivamente que la solución cumple con los requerimientos definidos. Esta estrategia no constituye una validación clínica ni certifica PULSO para uso médico, sino que busca validar el funcionamiento técnico y la utilidad del MVP dentro del alcance académico establecido.
